...
(async () => {
  try {
    ...
    const coll = mongoose.connection.collection('assignments');
    const res = await coll.updateOne(
      { assignmentId: ASSIGNMENT_ID },
      { $set: { questions, totalQuestions: questions.length, passingPercentage: 60 } }
    );
    console.log('Matched:', res.matchedCount, 'Modified:', res.modifiedCount);

    const doc = await coll.findOne({ assignmentId: ASSIGNMENT_ID });
    console.log('Questions in DB after update:', Array.isArray(doc?.questions) ? doc.questions.length : 0);
