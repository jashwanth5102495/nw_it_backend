/**
 * Append topics 13-15 to networking-beginner-2 and keep it study-only.
 */
...
async function main() {
  ...
  try {
    ...
    console.log('[INFO] Current title:', doc.title);
    console.log('[INFO] Current topics:', Array.isArray(doc.topics) ? doc.topics.length : 0);

    const existingTopics = Array.isArray(doc.topics) ? doc.topics : [];
    const existingIds = new Set(existingTopics.map(t => t.topicId));

    const topicsToAdd = newTopics.filter(t => !existingIds.has(t.topicId));
    const updatedTopics = existingTopics.concat(topicsToAdd);
