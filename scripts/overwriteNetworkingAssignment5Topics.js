...
const topics = [
  {
    ...
    examples: ['top','htop','vmstat 1','nvidia-smi','uptime']
  },
  {
    ...
    examples: ['systemctl status NetworkManager','systemctl restart sshd','systemctl enable firewalld','service networking restart','journalctl -u NetworkManager']
  },
  {
    ...
    examples: ['tcpdump -i eth0','tcpdump -i wlan0 port 53','tcpdump -nn -w capture.pcap','tcpdump host 192.168.1.10']
  },
  {
    ...
    examples: ['nc -zv 192.168.1.1 80','nc 192.168.1.1 22','nc -lvp 4444','echo "hello" | nc 192.168.1.10 5000']
  },
  {
    ...
    examples: ['ss -tulpn','ss -ant | grep ESTAB','ss -uap','ss -ltn']
  },
  {
    ...
    examples: ['tail -f /var/log/syslog','journalctl -f','tail -f /var/log/auth.log','journalctl -u sshd -f']
  }
];
...
async function run(){
  try{
    ...
    const exists = await Assignment.findOne({ assignmentId: ASSIGNMENT_ID });
    if(!exists){
    ...
    const result = await Assignment.findOneAndUpdate(
      ...
      { $set: { title: 'Assignment 5: System Services & Traffic Analysis', topics, updatedAt: new Date() } },
      { new: true }
    );

    console.log(`✅ Overwrote topics for '${ASSIGNMENT_ID}'`);
