...
(async () => {
  try {
    ...
    const assignment = await Assignment.findOne({ assignmentId: 'frontend-beginner-6' }).lean();

    if (!assignment) {
      console.log('❌ Assignment not found: frontend-beginner-6');
      process.exit(1);
    }
    ...
    (assignment.questions || []).forEach(q => {
      ...
      answers[q.questionId] = normalizedCorrect;
    });
